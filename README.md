...
- Фото профиля специалиста загружается в админке (Специалисты → «Фото профиля») и показывается в каталоге и на странице профиля. Оно входит в экспортируемый JSON.
- Чтобы контент стал общим для всех: заполните данные в админке → вкладка «Экспорт» → скопируйте JSON → пришлите его дизайнеру (в чат Claude), чтобы вшить в data.js. Фото портфолио (ячейки на странице профиля) пока пришлите файлами — они будут добавлены в код.
